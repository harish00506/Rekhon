...
interface QuickSetupRepository {
    ...
    /**
     * Observes the envelopes of the most recent budget period.
     ...
     * Result: emits on every change; an empty list when the user skipped quick setup — which the
     *         screen must render as an empty state, never as three zero envelopes.
     * Input:  [profileId]. Output: `Flow<List<BudgetEnvelope>>`.
     */
    fun observeLatestEnvelopes(profileId: String = DEFAULT_PROFILE_ID): Flow<List<BudgetEnvelope>>
 ...
 * Input:  the receiver; [profileId]; [nowUtcMillis]. Output: [RecurringRuleEntity].
 * Changelog: 2026-07-27 — Created for issue 2.3.
 */
