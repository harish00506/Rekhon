...
| [0031](docs/adr/0031-a-percentage-change-is-refused-when-net-worth-starts-at-or-below-zero.md) | 2026-08-30 | The net-worth trend reports a **percentage only when the series starts strictly above zero**, and a change only when there are two readings; both are `null` otherwise and the screen renders the absence | Net worth is routinely negative for a user with a home loan, and dividing by a negative flips the sign — -50,000 improving to -10,000 would report as "-80%", the opposite of what happened, under a chart visibly sloping upward. `abs(first)` fixes that case and silently breaks a series crossing zero, which is worse because it never announces itself; and a change measured over one reading claims a stability nobody observed |
