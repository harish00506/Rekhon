...
| [0032](docs/adr/0032-the-proxy-lives-in-this-repo-and-the-pin-set-is-two.md) | 2026-08-30 | The §22 proxy is a `:backend` Gradle module in this repo depending on `:core:model`; the wire contract is a file both suites read; the dev pin set is **two** keypairs and the rotation is actually exercised; `<debug-overrides>` closes the pinning test gap without the `SSLSocketFactory` seam ADR-0030 refused; `fx:` is ECB reference rates because RBI publishes no machine-readable feed, and policy rates are not served at all | One definition of `PriceKey` and of paise across both sides of the wire beats two that drift, which is what a TypeScript worker would have cost; a pin set whose second member has never been exercised is a rotation plan nobody has tested; debug-overrides cannot reach a release build, so the seam stays refused; and a repo rate is bps while this endpoint's only money field is paise, so bending one into the other is exactly the type confusion MNY-001/MNY-002 exist to prevent |
...
| **Ktor (server-core, server-netty)** | The §22 market-data proxy | Kotlin-native, so the wire types are written once in the language the rest of the repo already uses. Over a Cloudflare Worker, which is cheaper to host but puts `PriceKey` and paise in a second language with nothing keeping them in step — and forces pinning a CDN's leaf certificate, which rotates on the platform's schedule and not ours. **`ktor-server-content-negotiation`, `ktor-serialization-kotlinx-json` and `ktor-server-status-pages` are deliberately NOT pinned**: one route serialising with `Json.encodeToString` + `respondText` is fewer moving parts, and makes the exact bytes on the wire what the contract test asserts | `:backend` (6.7) |
| **`ktor-server-test-host`** | Driving the route over real HTTP in tests | `testApplication` exercises the production route function, status codes and headers. A direct call to the handler would test none of the routing, none of the query parsing and none of the `Cache-Control` | `:backend` (6.7) |
| **`logback-classic`** | The SLF4J binding on the server | Runtime-only, and it exists so a **failed start is visible** — without a binding the server says nothing at all when it cannot bind a port, and a runbook you cannot debug is not a runbook. Its config is also where "this service does not log request identifiers" is written down (no `CallLogging` plugin, no MDC in the pattern) | `:backend` (6.7) |
| **OkHttp + MockWebServer, reused server-side** | The proxy's upstream vendor fetches, and their tests | Chosen over `ktor-client-*`: both are already pinned for `:core:network`, so the server adds **no** new HTTP client or test double to the catalogue, and there is no second version to keep in step | `:backend` (6.7) |
