...
| **kotlinx.serialization** | JSON, incl. typed nav routes | One serializer for network, `ai/` fixtures and navigation, rather than Moshi/Gson alongside it | `:app`, `:core:database` |
...
| **Glance** | Home-screen widget | The only way to build a widget in Compose-shaped code; `RemoteViews` by hand is the alternative | **pinned, not yet consumed** — `:widget` is not built |
