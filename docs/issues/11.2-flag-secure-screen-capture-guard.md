# [11.2] FLAG_SECURE + screen-capture guard
...
Progress and phase checklist: [11.2-flag-secure-screen-capture-guard-tracker.md](11.2-flag-secure-screen-capture-guard-tracker.md)
...
- [ ] Dependencies (1.10) are complete or explicitly waived by the user
- [ ] Read `docs/issues/00-issue-workflow.md` and `CLAUDE.md`
- [ ] Read the SRS sections cited above (§23; FR-PRIV-*) and quote the requirement IDs back
...
1. Sensitive surfaces are capture-guarded (P-01).

### Label-specific

1. No hand-rolled crypto - Tink/Keystore only (SEC-003); no PII/amounts in release logs.
2. No off-device data flow without explicit, revocable, per-feature consent (P-01).
...
Full timestamped log: [11.2-flag-secure-screen-capture-guard-tracker.md](11.2-flag-secure-screen-capture-guard-tracker.md#verification-log).
...
No sub-tasks yet. Break into `docs/features/11.2-flag-secure-screen-capture-guard/tasks/` when this issue is too
