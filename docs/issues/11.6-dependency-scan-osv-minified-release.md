# [11.6] Dependency scan (OSV) + minified release

**Status:** Todo  
**Epic:** Epic 11 — Privacy, Security & Compliance  
**Priority:** High  
**Depends on:** 1.1  
**Assignee:** Harish G  
**Labels:** `security`, `ci`
...
**SRS sections:** §21.3; SEC-007  
**Design spec section:** §9 Build/CI/Release  
**Epic summary:** Key management, screen-capture guard, consents dashboard, crypto-shredding erase, DPDP alignment, dependency scanning, and the no-hand-rolled-crypto audit.

**Branch:** `feature/11-6-dependency-scan-osv-minified-release`

## Tracker

Progress and phase checklist: [11.6-dependency-scan-osv-minified-release-tracker.md](11.6-dependency-scan-osv-minified-release-tracker.md)
...
- [ ] The release build is R8-minified with PII/amount log stripping verified (§21.6).
...
- [ ] Read the SRS sections cited above (§21.3; SEC-007) and quote the requirement IDs back
...
1. Deps are scanned; the release strips PII/amount logs (SEC-007/§21.6).

### Label-specific

1. No hand-rolled crypto - Tink/Keystore only (SEC-003); no PII/amounts in release logs.
2. All CI gates green before merge; no new lint/detekt warnings (§21.6).

### Epic-specific

1. Privacy and security are fail-secure and consent-first; no hand-rolled crypto anywhere (P-01/SEC-003).
...
Full timestamped log: [11.6-dependency-scan-osv-minified-release-tracker.md](11.6-dependency-scan-osv-minified-release-tracker.md#verification-log).
...
No sub-tasks yet. Break into `docs/features/11.6-dependency-scan-osv-minified-release/tasks/` when this issue is too
