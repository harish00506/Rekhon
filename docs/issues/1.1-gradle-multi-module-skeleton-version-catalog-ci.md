# [1.1] Gradle multi-module skeleton + version catalog + CI
...
**SRS sections:** §21.2, §21.3; ARC-001, ARC-002  
**Design spec section:** §6 Module Structure, §9 Build/CI/Release  
...
**Branch:** `feature/1-1-gradle-multi-module-skeleton-version-catalog-ci`

## Tracker

Progress and phase checklist: [1.1-gradle-multi-module-skeleton-version-catalog-ci-tracker.md](1.1-gradle-multi-module-skeleton-version-catalog-ci-tracker.md)

---

## Acceptance Criteria

- [ ] Settings + module graph created for the §21.2 modules; dependency direction enforced `feature → domain → data/core` (ARC-001).
- [ ] `:core:model` and `:domain:*` are pure Kotlin/JVM with no Android imports, enforced by Gradle (ARC-002).
- [ ] One `libs.versions.toml` pins the §21.3 stack; `./gradlew help` and `assembleDebug` succeed.
...
- [ ] Dependencies (None) are complete or explicitly waived by the user
- [ ] Read `docs/issues/00-issue-workflow.md` and `CLAUDE.md`
- [ ] Read the SRS sections cited above (§21.2, §21.3; ARC-001, ARC-002) and quote the requirement IDs back
- [ ] Read the design spec section: **§6 Module Structure, §9 Build/CI/Release**
...
1. Module dependency is strictly one-way; feature modules never depend on each other (ARC-001).
2. Pin the stack from §21.3; no library swap without an ADR.
...
Full timestamped log: [1.1-gradle-multi-module-skeleton-version-catalog-ci-tracker.md](1.1-gradle-multi-module-skeleton-version-catalog-ci-tracker.md#verification-log).
...
Atomic task breakdown: [`1.1.1-gradle-multimodule-skeleton.md`](../features/1.1-project-skeleton/tasks/1.1.1-gradle-multimodule-skeleton.md).
