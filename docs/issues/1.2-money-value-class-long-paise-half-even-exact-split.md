# [1.2] Money value class (Long paise, HALF_EVEN, exact split)
...
**SRS sections:** MNY-001, MNY-002; §21.4  
...
**Branch:** `feature/1-2-money-value-class-long-paise-half-even-exact-split`

## Tracker

Progress and phase checklist: [1.2-money-value-class-long-paise-half-even-exact-split-tracker.md](1.2-money-value-class-long-paise-half-even-exact-split-tracker.md)
...
- [ ] Formatting uses Indian digit grouping (₹1,23,456.78); rates are integer basis points (MNY-002).
...
- [ ] Read the SRS sections cited above (MNY-001, MNY-002; §21.4) and quote the requirement IDs back
...
1. Any `Double`/`Float` on a monetary value fails review (MNY-001).
2. Splits must sum exactly; distribute the remainder deterministically.

### Label-specific

1. `:core:model` and `:domain:*` stay pure Kotlin/JVM - no Android imports (ARC-002).
2. Money is `Long` paise end-to-end; no `Double`/`Float` on a monetary value (MNY-001).
3. Splits distribute the remainder (largest-remainder, `Money.split`/`allocate`) so parts sum exactly; HALF_EVEN is for rounding a single value (`Money.percentOf`), and cannot make N parts sum to a whole.
...
Full timestamped log: [1.2-money-value-class-long-paise-half-even-exact-split-tracker.md](1.2-money-value-class-long-paise-half-even-exact-split-tracker.md#verification-log).
...
Atomic task breakdown: [`1.1.2-money-value-class.md`](../features/1.1-project-skeleton/tasks/1.1.2-money-value-class.md).
