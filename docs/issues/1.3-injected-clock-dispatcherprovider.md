# [1.3] Injected Clock + DispatcherProvider
...
**SRS sections:** TIM-001, TIM-002; ARC-006, §21.4  
...
**Branch:** `feature/1-3-injected-clock-dispatcherprovider`

## Tracker

Progress and phase checklist: [1.3-injected-clock-dispatcherprovider-tracker.md](1.3-injected-clock-dispatcherprovider-tracker.md)
...
- **TIM-001/002** - injected `Clock`, UTC epoch millis, ISO `LocalDate`; never `System.currentTimeMillis()` in domain.
...
- [ ] Read the SRS sections cited above (TIM-001, TIM-002; ARC-006, §21.4) and quote the requirement IDs back
...
1. Never `System.currentTimeMillis()` in domain - inject `Clock` (TIM-001).
2. Dispatchers are injected; `GlobalScope` is lint-banned (ARC-006).

### Label-specific

1. `:core:model` and `:domain:*` stay pure Kotlin/JVM - no Android imports (ARC-002).
2. Date/calendar logic goes through the injected `Clock`; never `System.currentTimeMillis()` in domain (TIM-001).
...
Full timestamped log: [1.3-injected-clock-dispatcherprovider-tracker.md](1.3-injected-clock-dispatcherprovider-tracker.md#verification-log).
...
Atomic task breakdown: [`1.1.3-clock-and-dispatchers.md`](../features/1.1-project-skeleton/tasks/1.1.3-clock-and-dispatchers.md).
